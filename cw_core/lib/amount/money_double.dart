...
extension ToMoney on double {
  Money? tryToMoney(Currency currency) =>
      Money.tryParse(_toSafeString(currency), currency);

  Money toMoney(Currency currency) =>
      Money.parse(_toSafeString(currency), currency);

  String _toSafeString(Currency currency) => trimTrailingFractionZeros(toStringAsFixed(20));
}
