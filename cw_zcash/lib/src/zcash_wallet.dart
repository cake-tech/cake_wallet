...
import 'package:cw_zcash/src/zcash_balance.dart';
import 'package:cw_zcash/src/zcash_transaction_info.dart';
import 'package:cw_zcash/src/zcash_transactions.dart';
import 'package:cw_zcash/src/zcash_wallet_addresses.dart';
import 'package:cw_zcash/src/models.dart';
import 'package:cw_zcash/src/pending_zcash_transaction.dart';
import 'package:cw_zcash/src/zcash_transaction_credentials.dart';
...
abstract class ZcashWalletBase extends WalletBase<ZcashBalance, ZcashTransactionHistory, ZcashTransactionInfo> with Store {
  ZcashWalletBase(super.walletInfo, super.derivationInfo, {
  ...
  Future<PendingTransaction> createTransaction(Object credentials) async {
    ...
    for (final output in creds.outputs) {
      int amount;
      if (output.sendAll) {
        amount = availableBalance - 10000; // 10000 is default fee.
        if (amount <= 0) {
        ...
        if (amount <= 0) {
          throw Exception('Invalid amount for output. Amount: ${output.cryptoAmount}, Formatted: ${output.formattedCryptoAmount}');
        }
      }
    ...
    final txPlan = await ZcashWalletService.runInDbMutex(() async => await WarpApi.prepareTx(
      coin,
      accountId,
      recipients,
  ...
  Future<Map<String, ZcashTransactionInfo>> fetchTransactions() async {
    ...
    final txs = await ZcashWalletService.runInDbMutex(() async => await WarpApi.getTxs(coin, accountId));
    ...
    for (final tx in txs) {
      ...
      final confirmations = tx.height > 0 && currentHeight > 0
  ...
  Future<void> _runWarpSync() async {
    ...
    try {
      ...
      void _cancelSyncIfShould(Timer t) {
        ...
        printV("Canceling sync! (ZcashWalletService.dbMutexQueue: ${ZcashWalletService.dbMutexQueue} > initialQueue: ${initialQueue}");
        WarpApi.cancelSync();
        t.cancel();
        _t = null;
      }
      unawaited(Future.delayed(Duration(seconds: 5)).then((_) {
        _t = Timer.periodic(Duration(milliseconds: 100), _cancelSyncIfShould);
      }));
      final result = await ZcashWalletService.runInDbMutex(() async => await WarpApi.warpSync(coin, accountId, true, 0, 100000, 0));
  ...
  void _startSyncStatusUpdates() {
    ...
    _syncStatusTimer = Timer.periodic(const Duration(milliseconds: 5000), (_) {
      _updateSyncStatus().catchError((e) {
  ...
  void _startPeriodicSync() {
    ...
    _stopPeriodicSync();
    _periodicSyncTimer = Timer.periodic(const Duration(seconds: 15), (_) async {
      try {
  ...
  Future<void> _updateSyncStatus() async {
    try {
      ...
      if (syncHeight <= 0) {
        if (syncStatus is! ConnectedSyncStatus && syncStatus is! ConnectingSyncStatus && syncStatus is! AttemptingSyncStatus) {
          syncStatus = ConnectedSyncStatus();
        }
        try {
          await updateBalance();
  ...
  String getDiversifiedAddress(int uaType, {DateTime? time}) {
    try {
  ...
  Future<void> _$autoShield() async {
    ...
    final bpConfirmed = WarpApi.getPoolBalances(coin, accountId, 11, false);
    ...
    final _txId = await ZcashWalletService.runInDbMutex(() async => await WarpApi.signAndBroadcast(
  ...
  Future<void> updateBalance() async {
    try {
      ...
      final confirmedPoolBalances = WarpApi.getPoolBalances(coin, accountId, 10, true);
  ...
  static Future<ZcashWallet> create(WalletCredentials credentials) async {
    await _init();
    ...
    final accountId = await _createNewZcashWallet(
  ...
  static Future<int> _createNewZcashWallet({
    required String name,
  ...
  static Future<int> _restoreZcashWalletFromSeed({
    ...
    final accountId = await ZcashWalletService.runInDbMutex(() async => await WarpApi.newAccount(coin, name, seed, 0));
    return accountId;
  }

  static Future<int?> getZcashAccountIdForName(String name) async {
  ...
  static Future<void> _saveAccountId(String name, int accountId) async {
    final wPath = await pathForWallet(name: name, type: _type);
    final f = File(wPath);
