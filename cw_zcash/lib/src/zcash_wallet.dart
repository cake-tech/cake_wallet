...
abstract class ZcashWalletBase extends WalletBase<ZcashBalance, ZcashTransactionHistory, ZcashTransactionInfo> with Store {
  ...
  Future<Map<String, ZcashTransactionInfo>> fetchTransactions() async {
    final txs = await WarpApi.getTxs(coin, accountId);
    ...
    for (final tx in txs) {
      ...
      int? txFee;
      if (tx.messages != null && tx.messages!.memos != null) {
        txFee = null;
      }
      
      final txId = tx.txId ?? tx.shortTxId ?? 'tx_${tx.id}_${tx.timestamp}';
  ...
  Future<void> rescan({required int height}) async {
    try {
      ...
      WarpApi.rescanFrom(coin, height);
      await startSync();
    } catch (e) {
  ...
  Future<void> updateTransactions() async {
    try {
      ...
      printV("Fetching transactions for account $accountId");
      final transactions = await fetchTransactions();
      
      printV("Fetched ${transactions.length} transactions");
      ...
      transactions.forEach((key, tx) {
        printV("Adding transaction: $key, amount: ${tx.amount}, direction: ${tx.direction}");
        transactionHistory.transactions[key] = tx;
      });
      await transactionHistory.save();
      printV("Transaction history updated, total transactions: ${transactionHistory.transactions.length}");
  ...
  Future<void> _runWarpSync() async {
    try {
      ...
      final result = await WarpApi.warpSync(coin, accountId, true, 0, 1000000, 0);
  ...
  Future<void> _$autoShield() async {
    ...
    final _txId = await WarpApi.signAndBroadcast(
  ...
  Future<void> updateBalance() async {
    try {
      ...
      final confirmedPoolBalances = WarpApi.getPoolBalances(coin, accountId, 10, false);
  ...
  WalletAddresses get walletAddresses {
    return ZcashWalletAddresses(accountId, walletInfo);
  }
  ...
  static Future<int> _createNewZcashWallet({
    ...
    final accountId = await WarpApi.newAccount(coin, name, seed, 0);
    return accountId;
  }
