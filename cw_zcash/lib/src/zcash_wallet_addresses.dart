...
abstract class ZcashWalletAddressesBase extends WalletAddresses with Store {
  ZcashWalletAddressesBase(this.accountId, WalletInfo walletInfo) : super(walletInfo) {
  ...
  @action
  Future<void> setAddressType(ZcashAddressType type) async {
  ...
  Future<void> init() async {
    ...
    if (addressInfos.isEmpty || addressesMap.isEmpty) {
      ...
      if (primaryAddr.isNotEmpty) {
        ...
        if (!addressInfos[unifiedType]!.any((info) => info.address == primaryAddr)) {
          addressInfos[unifiedType]!.add(WalletInfoAddressInfo(
      ...
      if (tAddr.isNotEmpty && !addressesMap.containsKey(tAddr)) {
        ...
        if (!addressInfos[transparentType]!.any((info) => info.address == tAddr)) {
          addressInfos[transparentType]!.add(WalletInfoAddressInfo(
            walletInfoId: walletInfo.internalId,
      ...
      if (sAddr.isNotEmpty && !addressesMap.containsKey(sAddr)) {
        ...
        if (!addressInfos[shieldedSaplingType]!.any((info) => info.address == sAddr)) {
          addressInfos[shieldedSaplingType]!.add(WalletInfoAddressInfo(
            walletInfoId: walletInfo.internalId,
      ...
      if (oAddr.isNotEmpty && !addressesMap.containsKey(oAddr)) {
        ...
        if (!addressInfos[shieldedOrchardType]!.any((info) => info.address == oAddr)) {
          addressInfos[shieldedOrchardType]!.add(WalletInfoAddressInfo(
            walletInfoId: walletInfo.internalId,
