...
CC=clang CXX=clang++ ./configure --prefix=${PREFIX} --host=${HOST} --enable-static --disable-shared --disable-flto --with-ssl=${PREFIX} \
	--with-libexpat=${PREFIX}  --enable-static-openssl --with-openssl-includes=${PREFIX} 
make -j$THREADS
make -j$THREADS install
done
