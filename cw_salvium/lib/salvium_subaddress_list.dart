...
part 'salvium_subaddress_list.g.dart';

class SalviumSubaddressList = SalviumSubaddressListBase
    with _$SalviumSubaddressList;

abstract class SalviumSubaddressListBase with Store {
  SalviumSubaddressListBase()
      : _isRefreshing = false,
        _isUpdating = false,
  ...
  Future<void> updateWithAutoGenerate({
    ...
    try {
      ...
      final newSubAddresses = await _getAllUnusedAddresses(
          accountIndex: accountIndex, label: defaultLabel);
  ...
  Future<List<Subaddress>> _getAllUnusedAddresses(
    ...
    if (allAddresses.isEmpty || _usedAddresses.contains(lastAddress)) {
      final isAddressUnused =
          await _newSubaddress(accountIndex: accountIndex, label: label);
      if (!isAddressUnused) {
    ...
    return allAddresses.map((s) {
      ...
      return Subaddress(
          ...
          txCount: s.txCount,
          label:
              id == 0 && label.toLowerCase() == 'Primary account'.toLowerCase()
                  ? 'Primary address'
                  : label);
    }).toList();
  }
