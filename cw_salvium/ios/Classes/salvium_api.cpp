...
extern "C"
{
    ...
    struct SalviumBalance
    {
        uint64_t amount;
        char *assetType;

        SalviumBalance(char *_assetType, uint64_t _amount)
        {
            amount = _amount;
    ...
    struct SalviumRate
    {
        ...
        SalviumRate(char *_assetType, uint64_t _rate)
        {
            rate = _rate;
            assetType = _assetType;
        }
    ...
    int64_t *get_full_balance(uint32_t account_index)
    {
        ...
        for (auto const& balance : accountBalance)
        {
            ...
            SalviumBalance *hb = new SalviumBalance(assetType, balance.second);
    ...
    int64_t *get_rate()
    {
        ...
        int64_t *salviumRates = (int64_t *)malloc(size * sizeof(int64_t));
        int i = 0;

        for (auto const& rate : rates)
        {   
            char *assetType = strdup(rate.first.c_str());
            SalviumRate *salviumRate = new SalviumRate(assetType, rate.second);
            salviumRates[i] = reinterpret_cast<int64_t>(salviumRate);
            i++;
        }
