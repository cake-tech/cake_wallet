...
// import 'package:monero/src/wallet2.dart' as monero; // Cyan's monero_c
// import 'package:cw_lws/src/wallet2.dart' as monero_lws; // vtnerd LWS
// import 'package:monero/src/wallet2.dart' as monero; // Full monero
...
Wallet2WalletManager wmPtr = (() {
  try {
    ...
    var printStarts = false;
    if (kDebugMode && debugMonero) {
      MoneroWalletManagerFactory().setLogLevel(4);
    }
    _wmPtr ??= MoneroWalletManagerFactory().getWalletManager() as Wallet2WalletManager?;
    if (kDebugMode && debugMonero) {
      MoneroWalletManagerFactory().setLogLevel(4);
    }
...
void createWallet(
  ...
  currentWallet = newW as Wallet2Wallet?;
...
Future<void> loadWallet({required String path, required String password, int nettype = 0}) async {
  if (openedWalletsByPath[path] != null) {
    txhistory = null;
    currentWallet = openedWalletsByPath[path]! as Wallet2Wallet?;
    return;
  }
  if (currentWallet == null || path != _lastOpenedWallet) {
    if (currentWallet != null) {
      ...
      Isolate.run(() {
        cw_lwsroot.Wallet_store(Pointer.fromAddress(addr));
      });
    }
    txhistory = null;
    ...
    final newWptrAddr = await Isolate.run(() {
      return cw_lwsroot.WalletManager_openWallet(Pointer.fromAddress(addr),
