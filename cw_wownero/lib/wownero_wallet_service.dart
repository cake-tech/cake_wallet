...
class WowneroNewWalletCredentials extends WalletCredentials {
  WowneroNewWalletCredentials({required String name, required this.language, required this.isPolyseed, String? password})
...
class WowneroWalletService extends WalletService<
  ...
  Future<WowneroWallet> restoreFromPolyseed(WowneroRestoreWalletFromSeedCredentials credentials) async {
    try {
      final path = await pathForWallet(name: credentials.name, type: getType());
  ...
  Future<WowneroWallet> _restoreFromPolyseed(String path, String password, Polyseed polyseed,
      WalletInfo walletInfo, PolyseedLang lang,
      {PolyseedCoin coin = PolyseedCoin.POLYSEED_WOWNERO, int? overrideHeight}) async {
    final height = overrideHeight ?? getWowneroHeightByDate(
