...
Future<List<Node>> loadDefaultSalviumNodes() async {
  final nodesRaw = await rootBundle.loadString('assets/salvium_node_list.yml');
  final loadedNodes = loadYaml(nodesRaw) as YamlList;
  final nodes = <Node>[];
...
Future<void> resetToDefault(Box<Node> nodeSource) async {
  ...
  final salviumNodes = await loadDefaultSalviumNodes();
