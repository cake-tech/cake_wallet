...
class FileInfo {
  ...
  FileInfo({required this.path, required this.size, required this.modified, this.content});
}
...
abstract class FileExplorerViewModelBase with Store {
  ...
  Future<String> getHexDiffDump(Uint8List oldBytes, Uint8List newBytes) async {
    ...
    buffer
        .writeln('Difference percentage: ${(diffBlocks / totalBlocks * 100).toStringAsFixed(2)}%');
  ...
  Future<String> compareFilesStreamBased(File currentFile, FileInfo oldInfo) async {
    ...
    if (newBytes.isNotEmpty) {
      ...
      buffer.writeln(
          '=== Detailed Binary Diff (${_formatSize(newData.length < oldBytes.length ? newData.length : oldBytes.length)}) ===');
  ...
  Future<void> loadSnapshots() async {
    try {
      ...
      final snapshotFiles = dir
          .listSync()
          .whereType<File>()
          .where((file) => file.path.endsWith('.snapshot'))
          .toList();
              ...
              (jsonData['files'] as Map).map((key, value) => MapEntry(
                    ...
                    FileInfo(
                      ...
                      content: value['content'] != null
                          ? base64Decode(value['content'] as String)
                          : null,
                    ),
                  )),
            );
    ...
    try {
      ...
      for (final entry in currentFiles.entries) {
        ...
        if (!snapshot.files.containsKey(path)) {
          ...
          } else if (old.size != current.size || !_areContentsEqual(old.content, current.content)) {
            fileChanges.add(FileChange(
              path: path,
  ...
  Future<String> getFileDiff(FileChange fileChange) async {
    if (fileChange.type == ChangeType.added) {
      ...
      buffer.writeln(
          'Difference: ${_formatSizeDifference(fileChange.newFile!.size - fileChange.oldFile!.size)}');
      buffer.writeln('');
      ...
      if (fileChange.oldFile!.content != null && fileChange.newFile!.content != null) {
        ...
        final diff =
            await getHexDiffDump(fileChange.oldFile!.content!, fileChange.newFile!.content!);
        buffer.write(diff);
      } else {
