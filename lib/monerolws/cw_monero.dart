...
class CWMoneroAccountList extends MoneroAccountList {
  ...
  ObservableList<Account> get accounts {
    ...
    final accounts = moneroWallet.walletAddresses.accountList.accounts.map((acc) => Account(id: acc.id, label: acc.label, balance: acc.balance)).toList();
  ...
  List<Account> getAll(Object wallet) {
    final moneroWallet = wallet as MoneroWallet;
    return moneroWallet.walletAddresses.accountList.getAll().map((acc) => Account(id: acc.id, label: acc.label, balance: acc.balance)).toList();
  }
  ...
  Future<void> setLabelAccount(Object wallet, {required int accountIndex, required String label}) async {
    final moneroWallet = wallet as MoneroWallet;
    moneroWallet.walletAddresses.accountList.setLabelAccount(accountIndex: accountIndex, label: label);
  }
}
...
class CWMoneroSubaddressList extends MoneroSubaddressList {
  ...
  List<Subaddress> getAll(Object wallet) {
    final moneroWallet = wallet as MoneroWallet;
    return moneroWallet.walletAddresses.subaddressList.getAll().map((sub) => Subaddress(id: sub.id, label: sub.label, address: sub.address, txCount: sub.txCount ?? 0, received: sub.balance ?? 'unknown')).toList();
  }

  @override
  Future<void> addSubaddress(Object wallet, {required int accountIndex, required String label}) async {
    final moneroWallet = wallet as MoneroWallet;
    return await moneroWallet.walletAddresses.subaddressList.addSubaddress(accountIndex: accountIndex, label: label);
  }

  @override
  Future<void> setLabelSubaddress(Object wallet, {required int accountIndex, required int addressIndex, required String label}) async {
    final moneroWallet = wallet as MoneroWallet;
    await moneroWallet.walletAddresses.subaddressList.setLabelSubaddress(accountIndex: accountIndex, addressIndex: addressIndex, label: label);
  }
}
...
class CWMonero extends Monero {
  ...
  @override
  TransactionPriority getMoneroTransactionPriorityAutomatic() => MoneroTransactionPriority.automatic;

  @override
  TransactionPriority deserializeMoneroTransactionPriority({required int raw}) => MoneroTransactionPriority.deserialize(raw: raw);
  ...
  WalletCredentials createMoneroRestoreWalletFromKeysCredentials({required String name, required String spendKey, required String viewKey, required String address, required String password, required String language, required int height}) =>
      MoneroRestoreWalletFromKeysCredentials(name: name, spendKey: spendKey, viewKey: viewKey, address: address, password: password, language: language, height: height);
  ...
  WalletCredentials createMoneroRestoreWalletFromHardwareCredentials({
    ...
    required ledger.LedgerConnection ledgerConnection,
      ...
      MoneroRestoreWalletFromHardwareCredentials(name: name, password: password, height: height, ledgerConnection: ledgerConnection);

  @override
  WalletCredentials createMoneroRestoreWalletFromSeedCredentials({required String name, required String password, required String passphrase, required int height, required String mnemonic}) => MoneroRestoreWalletFromSeedCredentials(name: name, password: password, passphrase: passphrase, height: height, mnemonic: mnemonic);
  ...
  WalletCredentials createMoneroNewWalletCredentials({
    ...
    String? mnemonic,
      ...
      MoneroNewWalletCredentials(
        ...
        password: password,
        language: language,
        seedType: seedType == 1 ? MoneroSeedType.polyseed : (seedType == 3 ? MoneroSeedType.bip39 : MoneroSeedType.legacy),
  ...
  Map<String, String> getKeys(Object wallet) {
    ...
    return <String, String>{'primaryAddress': keys.primaryAddress, 'privateSpendKey': keys.privateSpendKey, 'privateViewKey': keys.privateViewKey, 'publicSpendKey': keys.publicSpendKey, 'publicViewKey': keys.publicViewKey, 'passphrase': keys.passphrase};
  }
  ...
  @override
  Object createMoneroTransactionCreationCredentialsRaw({required List<OutputInfo> outputs, required TransactionPriority priority}) => MoneroTransactionCreationCredentials(outputs: outputs, priority: priority as MoneroTransactionPriority);

  @override
  String formatterMoneroAmountToString({required int amount}) => moneroAmountToString(amount: amount);

  @override
  double formatterMoneroAmountToDouble({required int amount}) => moneroAmountToDouble(amount: amount);
  ...
  @override
  WalletService createMoneroWalletService(Box<UnspentCoinsInfo> unspentCoinSource) => MoneroWalletService(unspentCoinSource);
