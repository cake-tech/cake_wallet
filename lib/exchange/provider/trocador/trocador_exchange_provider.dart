...
class TrocadorExchangeProvider extends ExchangeProvider {
  TrocadorExchangeProvider({this.useTorOnly = false, this.providerStates = const {}, super.proxyWrapper})
      : _lastUsedRateId = '',
        _provider = [];
  ...
  Future<ExchangeLimits> fetchLimits(
    ...
    final params = TrocadorCoinRequest(
    ...
    final responseJSON = (json.decode(response.body) as List<dynamic>).map((item)=>TrocadorCoin.fromJson(item as Map<String, dynamic>));
    ...
    return ExchangeLimits(
        min: Money.tryParse(coinJson.minimum, from), max: Money.tryParse(coinJson.maximum, from));
  }
  ...
  Future<ProviderRate> fetchRate(
      ...
      return ProviderRate(provider: description,
          rate: ExchangeRate.fromAmounts(Money.parse(responseJSON.amountFrom, from.currency),
  ...
  Future<Trade> createTrade({
    ...
    final params = TrocadorNewTradeRequest(id: request.isFixedRate ? _lastUsedRateId : null,
    ...
    final depositAmount = responseData.amountFrom.isEmpty ? request.depositAmount.cryptoAmount : Money.parse(responseData.amountFrom, request.depositCurrency);
    final payoutAmount = responseData.amountTo.isEmpty ? request.payoutAmount.cryptoAmount : Money.parse(responseData.amountTo, request.payoutCurrency);
  ...
  Future<Trade> findTradeById({required String id}) async {
    ...
    return proxyWrapper
      ...
      final from = CryptoCurrency.safeParseCurrencyFromString(responseData.tickerFrom, tag:fromTag);
