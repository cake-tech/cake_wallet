...
class StealthExExchangeProvider extends ExchangeProvider {
  ...
  Future<ExchangeLimits> fetchLimits(
    ...
    final body = StealthExRangeRequest(route: StealthExRoute(from: curFrom, to: curTo),
      ...
      final responseData = StealthExRange.fromJson(json.decode(response.body) as Map<String, dynamic>);
  ...
  Future<Trade> createTrade(
    ...
    DateTime? validUntil;
      ...
      final responseData = StealthExExchange.fromJson(json.decode(response.body) as Map<String, dynamic>);
      ...
      if (request.depositCurrency.tag != null && request.depositCurrency.title.toLowerCase() == responseData.deposit.symbol) {
      ...
      if (request.payoutCurrency.tag != null && request.payoutCurrency.title.toLowerCase() == responseData.withdrawal.symbol) {
  ...
  Future<Trade> findTradeById({required String id}) async {
    ...
    final from = CryptoCurrency.safeParseCurrencyFromString(responseData.deposit.symbol, tag: fromTag);
     // Parsing 'to' currency with network tag
    final toTag = responseData.withdrawal.network == "mainnet" ? null : responseData.withdrawal.network;
    final to = CryptoCurrency.safeParseCurrencyFromString(responseData.withdrawal.symbol, tag: toTag);
  ...
  Future<StealthExEstimate> getEstimatedExchangeAmount(
      {required CryptoCurrency from,
      required CryptoCurrency to,
