...
class UnreachableWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ...
      const Text(
        "Please screenshot and let me know immediately",
...
class SwapConfirmBottomWidget extends StatelessWidget {
      ...
      child: AnimatedSize(
        ...
        child: AnimatedSwitcher(
          ...
          child: Container(
            ...
            child: _buildBottomWidget(context, state),
          ),
        ),
      ),
    ),
  ...
  Widget _buildBottomWidget(BuildContext context, SwapState state) {
    if (state is SwapFailureState) {
    ...
    return switch (state.runtimeType) {
      ...
      SwapAwaitingWalletSwitch => LoadingBottomWidget(
        text:
            "${S.of(context).switching_to} ${(state as SwapAwaitingWalletSwitch).source.sourceWallet.name}",
      ),
      ...
      SwapAwaitingHardwareWallet => HardwareWalletProceedOnDeviceMessage(
        hardwareWalletType: (state as SwapAwaitingHardwareWallet).type,
      ),
      ...
      child: AnimatedSize(
        ...
        child: AnimatedSwitcher(
          ...
          child: Container(
            ...
            child: _buildBottomWidget(context, sendViewModel.state.runtimeType),
          ),
        ),
      ),
    ),
  ...
  Widget _buildBottomWidget(BuildContext context, Type state) {
    switch (state) {
      ...
      case IsExecutingState:
        return LoadingBottomWidget(text: "${S.of(context).generating_transaction}...");
      case FailureState:
        return TransactionErrorActions(errorText: (sendViewModel.state as FailureState).error);
      case IsDeviceSigningResponseState:
        return LoadingBottomWidget(text: "${S.of(context).signing_transaction}...");
