import 'dart:ui';

class CustomThemeColors {
  static const syncGreen = Color(0xFFFF12A439);
  static const syncYellow = Color(0xFFFFFB84E);

  static const warningOutlineColorDark = Color(0xFFFFB84E);
  static const warningContainerColorDark = Color(0xFF8E5800);
  static const warningContainerColorLight = Color(0xFFFFCC00);
  static const warningOutlineColorLight = Color(0xFF312938);
  static const warningOutlineColorBlack = Color(0xFFFFB84E);
  static const warningContainerColorBlack = Color(0xFF8E5800);

  static const backgroundGradientColorDark = Color(0xFF0F1A36);
  static const backgroundGradientColorLight = Color(0xFFE7E7FD);
  static const backgroundGradientColorBlack = Color(0xFF000000);

  static const cardGradientColorPrimaryDark = Color(0xFF2B3A67);
  static const cardGradientColorPrimaryLight = Color(0xFFFFFFFF);
  static const cardGradientColorPrimaryBlack = Color(0xFF202023);
  static const cardGradientColorSecondaryDark = Color(0xFF1C2A4F);
  static const cardGradientColorSecondaryLight = Color(0xFFF3F3FF);
  static const cardGradientColorSecondaryBlack = Color(0xFF181819);

  static const toggleKnobStateColorLight = Color(0xFFFFFFFF);
  static const toggleColorOffStateLight = Color(0xFFCACAE7);
  static const toggleColorOffStateDark = Color(0xFF3A4F88);
  static const toggleColorOffStateBlack = Color(0xFF3A4F88);
}
