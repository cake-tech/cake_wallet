...
#[allow(unused_variables)]
pub async fn start_ldk(start_args: Option<cli::LdkUserInfo>) {

	let args = match start_args {
		Some(user_args) => user_args,
		None => {
			match cli::parse_startup_args() {
...
use std::thread;
use std::sync::mpsc::{self, Sender, SyncSender, Receiver, sync_channel};
use lazy_static::lazy_static;
// use std::sync::Mutex;

// unsafe impl Sync for mpsc::Receiver<u64> {}
// static STATIC_CHANNEL: (mpsc::Sender<String>, mpsc::Receiver<String>) = mpsc::channel(); 
lazy_static! {
	static ref STATIC_CHANNEL: (SyncSender<String>, Mutex<Receiver<String>> ) = {
...
#[allow(unused_variables)]
pub fn static_channel(){
	let sender = &(*STATIC_CHANNEL).0;
	let receiver = &(*STATIC_CHANNEL).1;

	let sender_clone = sender.clone();
	thread::spawn(move || {
		for i in 1..10 {
			sender_clone.send(format!("msg from sender_clone: {}",i)).unwrap();
		} 
	});
	...
	thread::spawn(move || {
		for i in 1..20 {
			sender_clone2.send(format!("msg from sender_clone2: {}",i)).unwrap();
		} 
		sender_clone2.send("exit".to_string()).unwrap();
	});
...
pub fn threads_ldk(){
	...
	thread::spawn(move || {
		let msg = String::from("hello world from channel");
		tx.send(msg).unwrap();
	});
	...
	thread::spawn(move || {
		let vals = vec![
			"message from channel 1".to_string(),
			"message from channel 2".to_string(),
			"message from channel 3".to_string(),
		];
	...
	thread::spawn(move || {
		let vals = vec![
			"message from tx2_clone 1".to_string(),
			"message from tx2_clone 2".to_string(),
			"message from tx2_clone 3".to_string(),
		];

		for item in vals {
			tx2_clone.send(item).unwrap();
		};
	});
