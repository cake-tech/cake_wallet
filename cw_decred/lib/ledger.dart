...
Future<InputTx> createInputTx(Map<dynamic, dynamic> inp, int n, String walletName,
  ...
  for (var vout in vouts) {
    final atoms = (vout["value"] * 100000000).round();
...
Uint8List serializeTransactionOutputs(List<dynamic> outs) {
  ...
  for (final out in outs) {
    final atoms = (out["value"] * 100000000).round();
  ...
  try {
    ...
    for (var vout in vouts) {
      ...
      if (branch == 1) {
        final accountn = va["accountn"] ?? 0;
        final index = va["index"] ?? 0;
        changePath = "44'/42'/$accountn'/1/$index";
      }
    }
    ...
    for (final inp in ins) {
      final trustedInput = await getTrustedInput(connection, transformer,
          indexLookup: inp.vout, transaction: inp.tx);
    ...
    throw Exception('Ledger signing failed: $e}');
  } catch (e) {
  ...
  Future<List<HardwareAccountData>> getAvailableAccounts({int index = 0, int limit = 5}) async {
    ...
    for (final i in indexRange) {
      ...
      final createReq = {
        ...
        "network": isTestnet ? "testnet" : "mainnet", // TODO: Make it work on testnet.
        "depth": 2,
        "childn": i,
        "isprivate": false,
      };
