...
abstract class PivxWalletBase extends ElectrumWallet with Store {
  ...
  Future<ElectrumBalance> fetchBalances() async {
    ...
    // One unique scripthash per on-chain address. allAddresses can briefly hold
    // the same address under more than one record during discovery (a later
    // addAddresses de-dups via toSet, which is why the doubling self-heals);
    // summing a scripthash twice would double the transparent balance in that
    // window. Custom scripthash avoids SegWit exceptions.
    ...
    // Batch all balances in one message instead of one get_balance per
    // scripthash: the server throttles per message, not per method (verified).
    var balanceBySh = <String, Map<String, dynamic>>{};
    try {
      const chunkSize = 150;
    ...
    // getBatchBalance returns {} for a scripthash that errored, so one transient
    ...
    for (var i = 0; i < validScriptHashes.length; i++) {
      ...
      if (!hasKeys) {
        ...
        totalConfirmed += addressRecord.balance;
        continue;
      }
  ...
  // Keep the base's generic batch paths off: they derive confirmations from
  ...
  // received from a shielded tx. PIVX batches unspents itself in
  // fetchUnspentsForAddresses (below) and balances in fetchBalances, both with
  // height-derived confirmations, so this only guards paths PIVX doesn't
  // override.
  @override
  ...
  /// Batch every address's unspents in one message instead of one listunspent
  /// per address. The server throttles per message, not per method (verified),
  /// so this collapses N throttled round trips into ceil(N/150) batched ones.
  /// Custom PIVX scripthash + height-derived confirmations, so it avoids the
  /// base batch path's getScriptHash (throws for base58) and
  /// fetchTransactionInfoBatch (drops sapling-funded coins).
  @override
  Future<List<List<BitcoinUnspent>?>> fetchUnspentsForAddresses(
    ...
    try {
      for (var i = 0; i < uniqueSh.length; i += chunkSize) {
        ...
        unspentBySh.addAll(
            await electrumClient.getBatchUnspent(uniqueSh.sublist(i, end)));
      }
    } catch (e) {
    ...
    // Emit each scripthash's coins once. Duplicate address records for the same
    // scripthash (transient during discovery) get an empty list so utxos are
  ...
  /// register as a false incoming. A shielded-funded incoming (no transparent
  /// vin) counts all our addresses, since an external deshield can land on a
  /// change-branch receive address; a tx we funded transparently (our own t->z
  /// shield) restricts to non-change addresses so its change isn't miscounted.
  Future<ElectrumTransactionInfo?> _buildPivxIncomingFromVerbose(
    ...
    try {
      ...
      // A z->t deshield has zero transparent inputs (funded from the shielded
      // pool), so an incoming with no transparent vin that isn't one of our own
      // shielded spends (guarded above) is a genuine deshield receive: count
      // outputs to ALL our addresses, including a change-branch one. A tx WITH
      // transparent inputs could be our own t->z shield whose change lands on a
      // hidden address, so restrict to non-change addresses there.
      final vin = verbose['vin'];
      final hasTransparentInput = vin is List && vin.isNotEmpty;
      final matchAddresses = (hasTransparentInput
              ? walletAddresses.allAddresses.where((a) => !a.isHidden)
  ...
  Future<void> updateFeeRates() async {
    // PIVX uses a fixed fee policy (see feeRate), and the ElectrumX servers
    // don't serve blockchain.estimatefee. The base would fire 3 estimatefee
    // requests every sync and every minute for nothing, so no-op it.
  }
